...
@section('content')
<form action="{{ route('admin.authors.update', $author) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow max-w-lg">
    @csrf
    @method('PUT')
    ...
    <div class="mb-4">
        ...
        <input type="text" name="name" value="{{ old('name', $author->name) }}" class="w-full border rounded p-2">
    ...
    <div class="mb-4">
        ...
        <div id="photoPreview" class="mt-3">
            @if($author->profile_photo)
                <img src="{{ asset($author->profile_photo) }}" class="w-32 h-32 object-cover rounded-full shadow">
            @endif
        </div>
    </div>
    ...
    <div class="mb-4">
        <label class="inline-flex items-center">
            <input type="checkbox" name="is_popular" class="mr-2" {{ old('is_popular', $author->is_popular) ? 'checked' : '' }}>
